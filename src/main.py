...
subcommand_args = {
	"score": {
		...
		"args": {
			...
			("-mn", "--model-name"): {"type": str, "help": "The prefix that will be prepended to the filename like: <subcommand-dir>/<model-name>/fold_<fold>/variant_scores.tsv", "required": True},
			("-scdir", "--score-dir"): {"type": str, "help": "The directory to store scoring files. Used in this way: <score-dir>/<model-name>/fold_<fold>/variant_scores.tsv. Directory should already exist.", "required": True},
			("-s", "--chrom-sizes"): {"type": str, "help": "Path to TSV file with chromosome sizes", "required": True},
			("-f", "--fold"): {"type": int, "help": "The fold number to assign to this run.", "required": True},
			...
			("-scf", "--score-filenames"): { "nargs": '+', "help": "A list of file names of variant score files that will be used to overwrite the otherwise generated index filenames, and will be used like so: <score-dir>/<score-filename>.{tsv,h5} for each file in the list. Generally only needed if --no-scoring is used."},
	...
	"summarize": {
		...
		"args": {
			("-scdir", "--score-dir"): {"type": str, "help": "The directory to store scoring files. Used in this way: <score-dir>/<model-name>/fold_<fold>/variant_scores.tsv. Directory should already exist.", "required": True},
			("-sudir", "--summarize-dir"): { "type": str, "help": "The directory to store the summary file with average scores across folds. Used in this way: <summarize-dir>/<model-name>/variant_scores.tsv. Directory should already exist.", "required": True},
			...
			("-sc", "--schema"): { "type": str, "choices": ['bed', 'plink', 'plink2', 'chrombpnet', 'original'], "default": 'chrombpnet', "help": "Format for the input variants list."},
			("-scf", "--score-filenames"): { "nargs": '+', "help": "A list of file names of variant score files to be used to overwrite the otherwise generated index filenames, and will be used like so: <score-dir>/<score-filename>.{tsv,h5} for each file in the list. Generally only needed if --no-scoring is used."},
	...
	"annotate": {
		...
		"args": {
			("-fsdir", "--summarize-dir"): { "type": str, "help": "The directory to store the summary file with average scores across folds. Used in this way: <summarize-dir>/<model-name>/variant_scores.tsv. Directory should already exist.", "required": True},
			("-mn", "--model-name"): {"type": str, "help": "The prefix to be prepended to the filename like: <subcommand-dir>/<model-name>/fold_<fold>/variant_scores.tsv.", "required": True},
