...
def main(args):
    ...
    if args.input_metadata and args.annotate_output_paths and args.aggregate_output_path:
    ...
    elif args.annotate_output_paths and args.aggregate_output_path:
    ...
    for aggregate_out_path in aggregate_out_paths:
        ...
        if args.input_metadata:
            ...
            cur_metadata = pd.DataFrame({ANNOTATE_OUT_PATH_COL: args.annotate_output_paths, MODEL_ID_COL: args.annotate_output_paths, AGGREGATE_OUT_PATH_COL: [args.aggregate_output_path] * len(args.annotate_output_paths)})
        
        # Check that all the annotation files exist
        for cur_path in cur_metadata[ANNOTATE_OUT_PATH_COL]:
            if not os.path.exists(cur_path):
                raise FileNotFoundError(f"Annotation file {cur_path} not found.")
        
        agg_df = pd.read_csv(cur_metadata.iloc[0][ANNOTATE_OUT_PATH_COL], sep='\t')
        if args.add_temp_model_id:
            agg_df[MODEL_ID_COL] = cur_metadata.iloc[0][MODEL_ID_COL]

        # for cur_path in cur_metadata[ANNOTATE_OUT_PATH_COL].values[1:]:
        for _, row in cur_metadata.iloc[1:].iterrows():  
            cur_path = row[ANNOTATE_OUT_PATH_COL]
